...
public class TreeChecker {
	...
	//private static final double THRESHOLD_PVALUE = VCFConstants.GROUP_PVALUE;
...
//		}
	...
	public static ArrayList<ArrayList<Integer>> getCFMatrixPrime(String matrixFile){
		...
		for (Integer conflict: conflicts){
			matrixPrimeTrans.get(conflict.intValue() - counter);
	...
	public static int getGroupSizeThreshold(int n, int k){
		...
		for (; x > 1; x--){
			...
			if (pValue > VCFConstants.GROUP_PVALUE) break;
		}
	 ...
	 */
	private static Set<Integer> approxVertexCover(Set<Pair<Integer>> edges, Set<Integer> nodes, ArrayList<Integer> bCodeList, Set<Integer> currCover){
		/*
