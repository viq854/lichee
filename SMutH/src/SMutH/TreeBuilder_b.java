...
public class TreeBuilder_b {
	...
	public static void main(String[] args) {
		...
		for (int id=0; id<=9;id++)
		{
			//int sample = 2; 
			//int id = 6;
			String testName = sample+"_0"+id; //Raheleh
		String fileName = "../simulation_vcfs/tree_" + testName + ".raw.vcf";
			//String fileName = "../patient_vcfs/" + testName + ".vcf";
		VCFDatabase vcfDB = new VCFDatabase(fileName);
		vcfDB.generateMatrix("output.txt");
		//vcfDB.generateGATKFile("../editSNV/"+testName + ".GATK-output.txt");
		...
		if (matrixPrime != null) {
			...
			//	editSNV(conflicts, mutMap, vcfDB, i, testName);
				//SPBuilder spb = new SPBuilder(mutMap, noConflictMatrixPrime, conflicts);
				//SubpopShift();
			}
...
//		}
		...
		for (ArrayList<Integer> conflict: conflicts){
			...
			for (String conflictMatch: conflictMatchesList){
				...
				//int currMutConverted = vcfDB.getValidEntries(conflictStr, conflictMatch, 
							//possMutations, currFailCodes, pw);
			...
			for (int i = 0; i < conflictMatchesList.size(); i++){
				...
				for (int j = 0; j < matchEntries.size(); j++){
					...
					//System.out.println(i+1 + ". " + "Prob: " + probs.get(i));
					//System.out.println(probsToEntryMap.get(probs.get(i)).toString());
				}
				if (failCodes != null){
...
//		}
	 ...
	 */
	private void VisualizeTree(DirectedGraph<Integer, Integer> g) {
